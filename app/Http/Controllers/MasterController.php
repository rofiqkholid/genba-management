...
class MasterController extends Controller
{
    ...
    public function kpi_formula_table(Request $request)
    {
        ...
        $query = DB::table('KPIFormula')
            ...
            ->orderBy('KPIFormula.id', 'asc');

        if ($request->has('search') && !empty($request->search['value'])) {
            $searchValue = $request->search['value'];
            $query->where(function($q) use ($searchValue) {
                $q->where('KPIList.no_kpi', 'LIKE', "%{$searchValue}%")
                  ->orWhere('KPIList.objective', 'LIKE', "%{$searchValue}%")
                  ->orWhere('KPIFormula.formula_type', 'LIKE', "%{$searchValue}%")
                  ->orWhere('KPIFormula.result', 'LIKE', "%{$searchValue}%");
            });
        }

        $totalRecords = DB::table('KPIFormula')->count();
        $filteredRecords = $query->count();
        ...
        return response()->json([
            ...
            "data" => $data->map(function ($item, $key) use ($request, $canDelete) {
                ...
                if (!empty($item->result)) {
                    ...
                    for ($i = 1; $i <= 15; $i++) {
                        $val = $item->{'val_' . $i};
                        $numVal = ($val !== null && $val !== '') ? floatval($val) : 0;
                        $expr = preg_replace('/\bval_' . $i . '\b/', $numVal, $expr);
                    }

                    try {
                        $sanitized = preg_replace('/[^0-9+\-*\/().\s]/', '', $expr);
                        if (!empty($sanitized)) {
                            $valEvaluated = @eval('return (' . $sanitized . ');');
                            if ($valEvaluated !== false && is_numeric($valEvaluated)) {
                                $evaluatedResult = round($valEvaluated, 4);
                            }
                        }
                    } catch (\Throwable $t) {
                ...
                return [
                    "no" => $start + $key + 1,
                    "id" => $item->id,
                    "kpi" => $item->no_kpi . ' - ' . $item->objective,
                    ...
                    "action" => '<div class="flex items-center justify-start gap-2">
                                <button type="button" title="Edit" class="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200"
                                    onclick="handleEdit(this)"
                                    data-row="' . htmlspecialchars(json_encode($item)) . '">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path opacity="0.3" d="M10 4H21C21.6 4 22 4.4 22 5V7H10V4Z" fill="currentColor"></path>
                                    <path opacity="0.3" d="M10.3 15.3L11 14.6L8.70002 12.3C8.30002 11.9 7.7 11.9 7.3 12.3C6.9 12.7 6.9 13.3 7.3 13.7L10.3 16.7C9.9 16.3 9.9 15.7 10.3 15.3Z" fill="currentColor"></path><path d="M10.4 3.60001L12 6H21C21.6 6 22 6.4 22 7V19C22 19.6 21.6 20 21 20H3C2.4 20 2 19.6 2 19V4C2 3.4 2.4 3 3 3H9.20001C9.70001 3 10.2 3.20001 10.4 3.60001ZM11.7 16.7L16.7 11.7C17.1 11.3 17.1 10.7 16.7 10.3C16.3 9.89999 15.7 9.89999 15.3 10.3L11 14.6L8.70001 12.3C8.30001 11.9 7.69999 11.9 7.29999 12.3C6.89999 12.7 6.89999 13.3 7.29999 13.7L10.3 16.7C10.5 16.9 10.8 17 11 17C11.2 17 11.5 16.9 11.7 16.7Z" fill="currentColor"></path>
                                </svg>
    ...
    public function store_kpi_formula(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            for ($i = 1; $i <= 15; $i++) {
                $insertData['val_' . $i] = $request->input('val_' . $i);
            }

            DB::table('KPIFormula')->insert($insertData);

            return response()->json([
                'success' => true,
                'message' => 'KPI Formula added successfully.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function update_kpi_formula(Request $request)
    {
        $request->validate([
            'id' => 'required',
            'kpi_list_id' => 'required',
        ...
        try {
            ...
            for ($i = 1; $i <= 15; $i++) {
                $updateData['val_' . $i] = $request->input('val_' . $i);
            }

            DB::table('KPIFormula')->where('id', $request->id)->update($updateData);

            return response()->json([
                'success' => true,
                'message' => 'KPI Formula updated successfully.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function delete_kpi_formula(Request $request)
    {
        ...
        try {
            DB::table('KPIFormula')->where('id', $request->id)->delete();

            return response()->json([
                'success' => true,
                'message' => 'KPI Formula deleted successfully.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
