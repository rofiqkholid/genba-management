...
<div id="createModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-2xl transform transition-all">
            ...
            <form id="createForm" action="{{ route('master.check-item.store') }}" method="POST">
                @csrf
...
<div id="editModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-2xl transform transition-all">
            ...
            <form id="editForm" action="{{ route('master.check-item.update') }}" method="POST">
                @csrf
...
<script>
    $(document).ready(function() {
        var table = $('#checkItemTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('master.check-item.table') }}",
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
    ...
    function executeDelete() {
        ...
        $.ajax({
            url: "{{ route('master.check-item.delete') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                sys_id: id
